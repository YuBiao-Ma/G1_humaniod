...
from ..modules.ac_base import ActorCriticMlp as ActorCritic
...
class PPOLSE(PPO):
    ...
    def _compute_auxiliary_loss(self, batch: dict) -> dict:
        ...
        z_a, predicted_vel,predicted_contact,predicted_grad_vec,short_encode,long_encoded = encode_a[0], encode_a[1],  encode_a[2], encode_a[3], encode_a[4], encode_a[5]
        ...
        next_z_c, vel_c, contact, gravity = next_encode_c[0], encode_c[1], encode_c[2], encode_c[3]
        ...
        no_vel_training = vel_c.shape != predicted_vel.shape  # case of no vel rnn ablation study
        vel_loss = torch.tensor(0.0, device=vel_c.device) if no_vel_training else F.mse_loss(vel_c.squeeze(-2), predicted_vel)
        contact_loss = F.mse_loss(contact.squeeze(-2), predicted_contact)
        grad_vec_loss = F.mse_loss(gravity.squeeze(-2),predicted_grad_vec)
        return {
            "tar": triplet_loss * self.aux_loss_coef[0],
            "vel_mse": vel_loss * self.aux_loss_coef[-1],
            "contact_mse": contact_loss * self.aux_loss_coef[-1],
            "grad_vec_mse": grad_vec_loss * self.aux_loss_coef[-1],
        }
